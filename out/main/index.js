...
class FittrackeeApiClient extends events.EventEmitter {
  ...
  async makeRequest(method, endpoint, body) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const lib2 = isHttps ? https__namespace : http__namespace;
      const headers = {
      ...
      const req = lib2.request(options, (res) => {
        let data = "";
...
async function parseFitFile(filePath) {
  try {
    ...
    const workout = extractWorkoutData(buffer2, filePath);
...
function extractWorkoutData(buffer2, filePath) {
  try {
    const stats = fs__namespace.statSync(filePath);
    ...
    try {
      const ParserClass = FitParser;
      if (!ParserClass) {
        throw new Error("FIT.Parser not found in fit-file-parser");
      }
      parser = new ParserClass();
      const parsedData = parser.parse(buffer2);
